// =============================================================================
//
// Arduino - Flappy Bird clone (slightly modified build)
// --------------------------------------------
//
// Original project:
...
//
// This is a Flappy Bird clone for ATmega328 (Arduino UNO) using a
// Sainsmart 1.8" TFT display (ST7735).
//
// It features:
// - Intro screen
// - Gameplay similar to Flappy Bird
// - Game over screen with score display
//
// Hardware used:
// - Arduino UNO (ATmega328P)
// - ST7735 128x160 TFT display (SPI)
// - Push button input on D2
//
// Modifications in this version:
// - Button updated to INPUT_PULLUP (no external resistor required)
// - Simplified button wiring: D2 -> button -> GND
// - External pull-up resistor wiring removed
//
// Dependencies:
...
// bird size
#define BIRDW             12     // bird width
#define BIRDH             10     // bird height
#define BIRDW2            6     // half width
#define BIRDH2            5     // half height
// pipe size
#define PIPEW            12     // pipe width
#define GAPHEIGHT        50     // pipe gap height
// floor size
...
// background
const unsigned int BCKGRDCOL = TFT.color565(138,235,244); //const unsigned int BCKGRDCOL = TFT.color565(138,235,244);
// bird
...
{ C0, C0, C0, C0, C1, C1, C1, C1, C1, C1, C0, C0,
  C0, C0, C1, C1, C2, C2, C2, C2, C1, C3, C3, C1,
  C0, C1, C2, C2, C2, C2, C2, C2, C1, C3, C1, C1,
  C1, C2, C2, C2, C2, C2, C2, C2, C1, C3, C1, C1,
  C1, C1, C1, C1, C2, C2, C2, C2, C2, C1, C1, C1,
  C1, C2, C2, C2, C2, C2, C2, C2, C2, C4, C4, C4,
  C1, C2, C2, C2, C2, C2, C2, C2, C2, C4, C4, C4,
  C0, C1, C2, C2, C2, C2, C1, C5, C5, C5, C4, C0,
  C0, C0, C1, C1, C2, C1, C5, C5, C5, C5, C5, C0,
  C0, C0, C0, C0, C1, C1, C5, C5, C5, C5, C0, C0};
...
// ---------------
void setup() {
// initialize the push button on pin 2 as input with internal pull-up
pinMode(2, INPUT_PULLUP);

// internal pull-up enabled
// button wiring (CURRENT MODE):
// D2 ---- button ---- GND
// pressed = LOW, released = HIGH

// alternative wiring (EXTERNAL pull-up, NOT used here):
// 5V --- 10kΩ resistor --- D2 --- button --- GND
...
void game_loop() {
  ...
  while (1) {
    ...
    // ---------------
    tmpx = BIRDW-1;
    /* αυτος ο κωδικας δε παιζει σωστα
    do {
    ...
    */
    TFT.fillRect(bird.x + BIRDW, bird.old_y, BIRDW, BIRDH, BCKGRDCOL);
    for (tmpx = 0; tmpx < BIRDW; tmpx++) {
      for (tmpy = 0; tmpy < BIRDH; tmpy++) {
        unsigned int colour = birdcol[tmpx + (tmpy * BIRDW)];
        if (colour != BCKGRDCOL) { // simple transparency check
          drawPixel(bird.x + BIRDW + tmpx, bird.y + tmpy, colour);
        }
      }
    }
